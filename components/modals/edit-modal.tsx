...
import { z } from 'zod'
import { TaskType, labels, priorities, statuses } from '@/lib/validations/schema'
                        ...
                        {status_options.map((status, index) => (
                          <SelectItem key={index} value={status.value}>
                            <span className='flex items-center'>
                              <status.icon className='text-muted-foreground w-5 h-5 mr-2' />
                              {status.label}
                            </span>
                          </SelectItem>
                        ))}
                      ...
                      <SelectGroup>
                        {label_options.map((label, index) => (
                          <SelectItem key={index} value={label.value}>
                            <span className='flex items-center'>
                              <label.icon className='text-muted-foreground w-5 h-5 mr-2' />
                              {label.label}
                            </span>
                          </SelectItem>
                        ))}
                      ...
                      <SelectGroup>
                        {priority_options.map((priority, index) => (
                          <SelectItem key={index} value={priority.value}>
                            <span className='flex items-center'>
                              <priority.icon className='text-muted-foreground w-5 h-5 mr-2' />
                              {priority.label}
                            </span>
                          </SelectItem>
                        ))}
